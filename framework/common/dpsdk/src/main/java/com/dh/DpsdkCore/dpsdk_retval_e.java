 ...
 * @param DPSDK_RET_SERIALIZE_PDU_FAILED                = 2								 协议解析失败
 * @param DPSDK_RET_SENDDATA_FAILED                     = 3									 数据发送失败
 * @param DPSDK_RET_NET_CONNECT_FAILED                  = 4								 连接远端失败
 * @param DPSDK_RET_ALREADY_LOGIN                       = 5									 登录已存在
 * @param DPSDK_RET_ALREADY_LOGOUT                      = 6									 已登出
 * @param DPSDK_RET_SERIALIZE_XML_FAILED                = 7								Xml解析失败
 * @param DPSDK_RET_NODE_NO_RIGHT                       = 8									 无操作权限
 * @param DPSDK_RET_UNKNOW_DEVICE                       = 9									 未知设备
 * @param DPSDK_RET_UNKNOW_CHANNEL                      = 10									 未知通道
 * @param DPSDK_RET_DEVICE_OFFLINE                      = 11									 设备离线
 * @param DPSDK_RET_RTSP_CONNECT_FAILED                 = 12								rtsp连接失败
 * @param DPSDK_RET_RTP_LISTEN_FAILED                   = 13								rtp监听失败
 * @param DPSDK_RET_RTP_CONNECT_FAILED                  = 14								rtp连接失败
 * @param DPSDK_RET_PARSE_TRANSPORT_FAILED              = 15							rtsp解析端口失败
 * @param DPSDK_RET_ALREADY_CANCELED                    = 16								 命令已被取消
 * @param DPSDK_RET_UNKNOWN_SESSION                     = 17									无效的session fixme:检查已有情况统一为此应答
 * @param DPSDK_RET_TALK_INNVALIDPARAM                  = 18								 对讲参数有误
 * @param DPSDK_RET_TALK_NOT_SUPPORT                    = 19								 对讲不支持
 * @param DPSDK_RET_DMSSESSION_NOT_FOUND                = 20							 没找到对应的dmsSession
 * @param DPSDK_RET_ADSSESSION_NOT_FOUND                = 21							 未找到ADS
 * @param DPSDK_RET_ADSSESSION_NOT_LOGIN                = 22							 未登录成功ADS
 * @param DPSDK_RET_DATABASE_NO_RECORD                  = 23								 没有找到ALARM相关记录
 * @param DPSDK_RET_ALARMTYPE_NOT_MATCH                 = 24								 报警类型不匹配
 * @param DPSDK_RET_PLAYBACK_SESSION_NOT_FOUND          = 25						 无法找到回放会话
 * @param DPSDK_RET_PLAYBACK_UNKNOW_PLAYTYPE            = 26						 无法区分播放类型
 * @param DPSDK_RET_PLAYBACK_NETSDK_SEEK_FAIL           = 27						 网络SDK的seek命令失败
 * @param DPSDK_RET_PLAYBACK_SEEK_SIZE_ERROR            = 28						 回放定位的size错误
 * @param DPSDK_RET_PLAYBACK_SEEK_TIME_ERROR            = 29						 回放定位的时间错误
 * @param DPSDK_RET_PLAYBACK_DEVICE_NOT_FOUND           = 30						 回放时没有找到想要的设备
 * @param DPSDK_RET_PLAYBACK_PLAY_REPEAT                = 31							 该通道只允许回放一路
 * @param DPSDK_RET_INVALID_CONFIGTYPE                  = 32								 该配置信息的类型不支持
 * @param DPSDK_RET_UNSUPPORTED_STREAM                  = 33								 该码流类型不支持
 * @param DPSDK_RET_PLAYBACK_NOTFIND_DEV                = 34							 设备信息未找到
 * @param DPSDK_RET_PLAYBACK_PARAM                      = 35									 回放参数错误
 * @param DPSDK_RET_PLAYBACK_PARAM_CAMERA               = 36							 回放未知摄像头
 * @param DPSDK_RET_FTPSERVER_NOT_FIND                  = 37								FTP服务器没找到
 * @param DPSDK_RET_FTPCURL_ERROR                       = 38									 调用curl返回错误
 * @param DPSDK_RET_FTPCURL_LOCALFILE_NOT_OPEN          = 39						 调用curl返回未找到本地文件
 * @param DPSDK_RET_FTPCURL_LOCALFILE_NOT_READ          = 40						 调用curl返回打开本地文件出错
 * @param DPSDK_RET_FTPCURL_FTPFILE_ERROR               = 41							 调用curl返回操作远程文件出错
 * @param DPSDK_RET_FTPCURL_FREE_ERROR                  = 42								 调用curl返回释放出错
 * @param DPSDK_RET_PCS_ERROR                           = 43                        				PCS发生错误
 * @param DPSDK_RET_PES_ERROR                           = 44                        				PES发生错误
 * @param DPSDK_RET_PES_NOT_LOGIN                       = 45									 未登陆到PES
 * @param DPSDK_RET_PES_ENCODE_AHSOTCMD_FAILED          = 46       				 报警主机命令组包失败
 * @param DPSDK_RET_PES_ENCODE_DCTRL_FAILED             = 47          				 门禁命令组包失败
...
public class dpsdk_retval_e {
    ...
    public static final int DPSDK_RET_SERIALIZE_PDU_FAILED = 2;                            // 协议解析失败
    public static final int DPSDK_RET_SENDDATA_FAILED = 3;                                    // 数据发送失败
    public static final int DPSDK_RET_NET_CONNECT_FAILED = 4;                                // 连接远端失败
    public static final int DPSDK_RET_ALREADY_LOGIN = 5;                                    // 登录已存在
    public static final int DPSDK_RET_ALREADY_LOGOUT = 6;                                    // 已登出
    public static final int DPSDK_RET_SERIALIZE_XML_FAILED = 7;                            // Xml解析失败
    public static final int DPSDK_RET_NODE_NO_RIGHT = 8;                                    // 无操作权限
    public static final int DPSDK_RET_UNKNOW_DEVICE = 9;                                    // 未知设备
    public static final int DPSDK_RET_UNKNOW_CHANNEL = 10;                                    // 未知通道
    public static final int DPSDK_RET_DEVICE_OFFLINE = 11;                                    // 设备离线
    public static final int DPSDK_RET_RTSP_CONNECT_FAILED = 12;                            // rtsp连接失败
    public static final int DPSDK_RET_RTP_LISTEN_FAILED = 13;                                // rtp监听失败
    public static final int DPSDK_RET_RTP_CONNECT_FAILED = 14;                                // rtp连接失败
    public static final int DPSDK_RET_PARSE_TRANSPORT_FAILED = 15;                            // rtsp解析端口失败
    public static final int DPSDK_RET_ALREADY_CANCELED = 16;                                // 命令已被取消
    public static final int DPSDK_RET_UNKNOWN_SESSION = 17;                                // 无效的session fixme:检查已有情况统一为此应答

    public static final int DPSDK_RET_TALK_INNVALIDPARAM = 18;                                // 对讲参数有误
    public static final int DPSDK_RET_TALK_NOT_SUPPORT = 19;                                // 对讲不支持
    public static final int DPSDK_RET_DMSSESSION_NOT_FOUND = 20;                            // 没找到对应的dmsSession

    public static final int DPSDK_RET_ADSSESSION_NOT_FOUND = 21;                            // 未找到ADS
    public static final int DPSDK_RET_ADSSESSION_NOT_LOGIN = 22;                            // 未登录成功ADS
    public static final int DPSDK_RET_DATABASE_NO_RECORD = 23;                                // 没有找到ALARM相关记录
    public static final int DPSDK_RET_ALARMTYPE_NOT_MATCH = 24;                            // 报警类型不匹配

    public static final int DPSDK_RET_PLAYBACK_SESSION_NOT_FOUND = 25;                        // 无法找到回放会话
    public static final int DPSDK_RET_PLAYBACK_UNKNOW_PLAYTYPE = 26;                        // 无法区分播放类型
    public static final int DPSDK_RET_PLAYBACK_NETSDK_SEEK_FAIL = 27;                        // 网络SDK的seek命令失败
    public static final int DPSDK_RET_PLAYBACK_SEEK_SIZE_ERROR = 28;                        // 回放定位的size错误
    public static final int DPSDK_RET_PLAYBACK_SEEK_TIME_ERROR = 29;                        // 回放定位的时间错误
    public static final int DPSDK_RET_PLAYBACK_DEVICE_NOT_FOUND = 30;                        // 回放时没有找到想要的设备
    public static final int DPSDK_RET_PLAYBACK_PLAY_REPEAT = 31;                            // 该通道只允许回放一路
    public static final int DPSDK_RET_INVALID_CONFIGTYPE = 32;                                // 该配置信息的类型不支持
    public static final int DPSDK_RET_UNSUPPORTED_STREAM = 33;                                // 该码流类型不支持
    public static final int DPSDK_RET_PLAYBACK_NOTFIND_DEV = 34;                            // 设备信息未找到
    public static final int DPSDK_RET_PLAYBACK_PARAM = 35;                                    // 回放参数错误
    public static final int DPSDK_RET_PLAYBACK_PARAM_CAMERA = 36;                            // 回放未知摄像头
    public static final int DPSDK_RET_FTPSERVER_NOT_FIND = 37;                                // FTP服务器没找到
    public static final int DPSDK_RET_FTPCURL_ERROR = 38;                                    // 调用curl返回错误
    public static final int DPSDK_RET_FTPCURL_LOCALFILE_NOT_OPEN = 39;                        // 调用curl返回未找到本地文件
    public static final int DPSDK_RET_FTPCURL_LOCALFILE_NOT_READ = 40;                        // 调用curl返回打开本地文件出错
    public static final int DPSDK_RET_FTPCURL_FTPFILE_ERROR = 41;                            // 调用curl返回操作远程文件出错
    public static final int DPSDK_RET_FTPCURL_FREE_ERROR = 42;                                // 调用curl返回释放出错
    public static final int DPSDK_RET_PCS_ERROR = 43;                                        // PCS发生错误
    public static final int DPSDK_RET_PES_ERROR = 44;                                        // PES发生错误
    public static final int DPSDK_RET_PES_NOT_LOGIN = 45;                                    // 未登陆到PES
    public static final int DPSDK_RET_PES_ENCODE_AHSOTCMD_FAILED = 46;                    // 报警主机命令组包失败
    public static final int DPSDK_RET_PES_ENCODE_DCTRL_FAILED = 47;                        // 门禁命令组包失败
